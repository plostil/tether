...
- [x] Hosted app on GitHub Pages (`/tether/app/`) with no backend: the page picks a transport (`apps/web/src/app/transport.ts`) — an explicit broker URL, else the in-tab loopback hub when no broker answers — so the demo runs the real handshake and WebRTC path on a static page. Real pairing still needs the broker (`docs/DEPLOY.md`). `npm run check:live -w apps/web` is the post-deploy acceptance check.
